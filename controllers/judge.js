...
exports.quesSubmitController = async (req, res) => {
    ...
    try {
        ...
        const outputPath = path.join(__dirname, '../', 'compiled_codes', filename.split('.')[0]);
        const testInput = path.join(__dirname, '../', 'test_input', `${filename.split('.')[0]}.txt`);
        const testOutput = path.join(__dirname, '../', 'test_output', `${filename.split('.')[0]}.txt`);
        ...
        try {
            ...
            return res.json({ status: false, message: "Compilation error", error: compileErr.message });
        }
        ...
        for (let i = 0; i < testCases.length; i++) {
            ...
            try {
                await runTestCase(outputPath, testCase.input, testCase.output, result.timeLimit);
                passed++;
                if (passed === testCases.length) {
                    return res.json({ status: true, message: `All  ${passed + 1} test cases passed` });
                }
            } catch (testErr) {
                if(testErr.message === 'TLE')
                {
                    return res.json({ status: false, message: `Time Limit Exceeded on test case ${passed+1}` });
                }
                console.error('Test case failed:', testErr);
                return res.json({ status: false, message: `Test case ${passed + 1} failed`, error: testErr.message });
            }
        }
